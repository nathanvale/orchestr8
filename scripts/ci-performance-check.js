...
Usage: node scripts/ci-performance-check.js [options]
...
Thresholds:
${Object.entries(THRESHOLDS).map(([job, threshold]) => 
  `  ${job}: ${threshold}s`
).join('\n')}

Examples:
  node scripts/ci-performance-check.js --job lint --duration 45
  node scripts/ci-performance-check.js --job build --duration 200 -v
`)
}
...
function checkPerformance(job, duration) {
  ...
  if (!threshold) {
    console.warn(`${colors.yellow}⚠️  Warning: No threshold defined for job '${job}'${colors.reset}`)
 ...
 */
function generateReport(job, duration, result, verbose) {
 ...
 */
function getOptimizationTips(job) {
  const tips = {
    lint: [
 ...
 */
function savePerformanceData(job, duration, passed) {
  const dataFile = path.join(process.cwd(), '.ci-performance-data.json')
  let data = { runs: [] }
  ...
  data.runs = []
  Object.entries(jobRuns).forEach(([jobName, runs]) => {
...
function main() {
  ...
  if (!options.job || options.duration === null) {
