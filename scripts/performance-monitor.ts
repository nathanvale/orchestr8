...
function loadPerformanceData(): PerformanceData {
  ...
  try {
    ...
    return {
      runs: [],
      summary: { avgCacheHitRate: 0, avgBuildTime: 0, avgInstallTime: 0, totalRuns: 0 },
    }
  }
}
    ...
    try {
      if (process.platform === 'darwin' || process.platform === 'linux') {
        const loadAvg = execSync(
          'node -e "console.log(os.loadavg()[0])" -e "const os = require(\'os\')"',
          {
            encoding: 'utf-8',
          },
        ).trim()
...
function measureBuildPerformance(): BuildPerformance {
  ...
  try {
    ...
    results.estimatedBuildTime = hasComplexBuild
      ? Math.random() * 300 + 180
      : Math.random() * 60 + 30
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error'
...
function checkThresholds(perfData: {
  ...
  if (perfData.cache.restoreTime > THRESHOLDS.maxCacheRestoreTime) {
    issues.push(
      `Cache restore time (${perfData.cache.restoreTime}s) exceeds threshold (${THRESHOLDS.maxCacheRestoreTime}s)`,
    )
  }
  ...
  if (perfData.build.estimatedBuildTime > THRESHOLDS.maxBuildTime) {
    issues.push(
      `Estimated build time (${Math.round(perfData.build.estimatedBuildTime)}s) exceeds threshold (${THRESHOLDS.maxBuildTime}s)`,
    )
  }

  if (perfData.build.installTime > THRESHOLDS.maxInstallTime) {
    issues.push(
      `Install time (${Math.round(perfData.build.installTime)}s) exceeds threshold (${THRESHOLDS.maxInstallTime}s)`,
    )
  }
  ...
  if (perfData.system.memory > THRESHOLDS.maxMemoryUsage) {
    issues.push(
      `Memory usage (${perfData.system.memory}MB) exceeds threshold (${THRESHOLDS.maxMemoryUsage}MB)`,
    )
  }

  if (perfData.system.cpu > THRESHOLDS.maxCpuUsage) {
    warnings.push(
      `CPU usage (${perfData.system.cpu}%) is high (threshold: ${THRESHOLDS.maxCpuUsage}%)`,
    )
  }
...
function displayReport(report: PerformanceReport): void {
  ...
  console.log('\n📊 Current Performance:')
  console.log(
    `  Cache Hit: ${report.current.cache.cacheHit ? '✅' : '❌'} ${report.current.cache.cacheHit}`,
  )
  ...
  if (report.historical) {
    ...
    console.log(
      `  Trend: ${report.historical.trend === 'GOOD' ? '✅' : '⚠️'} ${report.historical.trend}`,
    )
  }
