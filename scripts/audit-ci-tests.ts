...
/**
 * Script to inventory and audit all CI-related test files
 * This implements the TestAuditor functionality from test-audit.integration.test.ts
 */
...
class CITestAuditor {
  ...
  private isCIRelated(content: string, filePath: string): boolean {
  ...
  private extractCIAspects(content: string): string[] {
  ...
  generateReport(files: TestFile[]): void {
