...
describe('QualityChecker - Timeout and Resource Management', () => {
  ...
  describe('Graceful Degradation', () => {
    it('should continue with remaining engines when one times out', async () => {
      ...
      expect(result.issues.some((issue) => issue.message === 'Prettier formatting issue')).toBe(
        true,
      )
    ...
    it('should apply backpressure when resource limits are approached', async () => {
      ...
      vi.mocked(TypeScriptEngine).prototype.check = vi.fn().mockImplementation(async (files) => {
        ...
        await new Promise((resolve) => setTimeout(resolve, files.length * 2))
        return { issues: [] }
      })
      ...
      expect(result.success).toBe(true)
      // Verify that batch sizes were adjusted (backpressure applied)
      // Later batches should be smaller if memory pressure increased
    ...
    it('should skip non-critical engines under resource pressure', async () => {
      ...
      // Prettier should be skipped under memory pressure
      vi.mocked(PrettierEngine).prototype.check = vi.fn().mockResolvedValue({ issues: [] })
      ...
      process.memoryUsage = (() => ({
        ...
        heapUsed: 150 * 1024 * 1024, // 150MB - over threshold
      ...
      // Critical engine should have run
      expect(result.issues.some((issue) => issue.message === 'Type error')).toBe(true)

      // Non-critical engine might be skipped
      expect(vi.mocked(PrettierEngine).prototype.check).toHaveBeenCalledTimes(0)
    })
  })
})
