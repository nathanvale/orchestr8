...
describe.skipIf(!shouldRun)(
  'PostgresContainer Comprehensive Tests' + (skipReason ? ' (skipped: ' + skipReason + ')' : ''),
  () => {
    ...
    describe('Container Lifecycle', () => {
      it('should start container and establish connection', async () => {
        ...
        // Accept both postgres:// and postgresql:// schemes (both are valid)
        expect(container.getConnectionString()).toMatch(
          /^postgres(ql)?:\/\/test_user:test_pass@.*:\d+\/test_db$/,
        )
