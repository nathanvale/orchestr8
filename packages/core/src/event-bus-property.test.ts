...
describe('Event Bus Property Tests', () => {
  describe('Circular Buffer Invariants', () => {
    it('should never exceed configured queue size', { timeout: 30000 }, async () => {
      await fc.assert(
        fc.asyncProperty(
    ...
    it('should maintain FIFO order within capacity', { timeout: 30000 }, async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          async (queueSize, eventIds) => {
            ...
            if (receivedIds.length > 1) {
              ...
              for (let i = 1; i < receivedIds.length; i++) {
                const currentFirstIndex = firstOccurrenceMap.get(receivedIds[i]) ?? 0
                const previousFirstIndex = firstOccurrenceMap.get(receivedIds[i - 1]) ?? 0
                // Current event should not come from before the previous event's first occurrence
                expect(currentFirstIndex).toBeGreaterThanOrEqual(previousFirstIndex)
              }
            }
          },
        ),
    ...
    it('should accurately track dropped events', { timeout: 30000 }, async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          async (queueSize, eventCount) => {
            ...
            eventBus.on('drop.test' as OrchestrationEvent['type'], async () => {
            ...
            const totalAccounted =
              processedCount + metrics.queueSize + metrics.droppedCount
            expect(totalAccounted).toBe(eventCount)
          },
        ),
        { numRuns: 10 },
      )
  ...
  describe('Event Isolation Properties', () => {
    it('should isolate event data between listeners (shallow clone behavior)', { timeout: 30000 }, async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.record({
          ...
          async (originalEvent) => {
            ...
            if (listenerModifiedData && originalEvent.data?.nested) {
              ...
              expect((originalEvent.data.nested as any).modified).toBe(true)
            }
          },
        ),
        { numRuns: 10 },
    ...
    it('should deliver events to all listeners', { timeout: 30000 }, async () => {
      await fc.assert(
        fc.asyncProperty(
  ...
  describe('Metrics Consistency Properties', () => {
    it('should maintain consistent metrics relationships', { timeout: 30000 }, async () => {
      await fc.assert(
        fc.asyncProperty(
          eventBusConfigArb,
          ...
          async (config, events) => {
            ...
            let processedCount = 0
            if (events.length > 0) {
              eventBus.on(events[0].type as OrchestrationEvent['type'], () => {
                processedCount++
              })
            }
    ...
    it('should account for all events (processed + queued + dropped = emitted)', { timeout: 30000 }, async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.integer({ min: 10, max: 100 }), // queue size
          ...
          async (queueSize, eventCount) => {
            ...
            const totalAccounted = totalProcessed + totalDropped

            // All emitted events should be either processed or dropped
            expect(totalAccounted).toBe(eventCount)
          },
        ),
        { numRuns: 10 },
  ...
  describe('Configuration Properties', () => {
    it('should respect all configuration boundaries', { timeout: 30000 }, async () => {
      await fc.assert(
        fc.asyncProperty(eventBusConfigArb, async (config) => {
          ...
          const listenersToAdd = Math.min(config.maxListenersPerEvent + 10, 1000)
          ...
          expect(metrics.highWaterMark).toBeLessThanOrEqual(config.maxQueueSize)
  ...
  describe('Concurrency Properties', () => {
    it('should handle concurrent emissions correctly', { timeout: 30000 }, async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.integer({ min: 10, max: 100 }), // queue size
          ...
          async (queueSize, emitterCount, eventsPerEmitter) => {
            ...
            const emitters = Array.from({ length: emitterCount }, (_, source) =>
              (async () => {
            ...
            for (const [_source, ids] of receivedBySource) {
            ...
            const totalReceived = Array.from(receivedBySource.values()).reduce(
