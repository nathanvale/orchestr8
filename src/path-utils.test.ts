...
const FILE_TXT = 'file.txt'
const FOLDER = 'folder'
const SUB = 'sub'
const POSIX_FILE_PATH = `${FOLDER}/${SUB}/${FILE_TXT}`
const MIXED_PATH = `${FOLDER}/${SUB}\\${FILE_TXT}`
...
describe('cross-platform path utilities', () => {
  ...
  describe('normalizePath', () => {
    ...
    test('normalizes mixed separators to platform separator', () => {
      const result = normalizePath(MIXED_PATH)
  ...
  describe('toPosixPath', () => {
    test('converts backslashes to forward slashes', () => {
      expect(toPosixPath(`${FOLDER}\\${SUB}\\${FILE_TXT}`)).toBe(POSIX_FILE_PATH)
    })

    test('preserves existing forward slashes', () => {
      expect(toPosixPath(POSIX_FILE_PATH)).toBe(POSIX_FILE_PATH)
    })
