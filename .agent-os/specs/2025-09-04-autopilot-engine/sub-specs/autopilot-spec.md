    ...
    switch (issue.rule) {
      case 'no-console':
        ...
        if (isTestFile || isDevFile) {
          return {
            safe: false,
            reason: 'Console might be intentional in test/dev',
          }
        }
        return { safe: true }
...
describe('Autopilot', () => {
  ...
  describe('Unsafe Rules', () => {
    it('should not auto-fix undefined variables', () => {
      const result: CheckResult = {
        filePath: 'test.ts',
        issues: [{ rule: 'no-undef', fixable: false, file: 'test.ts' }],
        hasErrors: true,
        hasWarnings: false,
        fixable: false,
      }
  ...
  describe('Context-Dependent Rules', () => {
    ...
    it('should remove console in production files', () => {
      const result: CheckResult = {
        ...
        issues: [{ rule: 'no-console', fixable: true, file: 'api/handler.ts' }],
        hasErrors: false,
        hasWarnings: true,
        fixable: true,
      }
...
The result will be an Autopilot that can safely fix 80%+ of common issues while
never breaking code.
