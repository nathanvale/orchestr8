...
- Use consistent naming conventions
  ...
  **IMPORTANT**: Wallaby MCP tools are ONLY for `.unit.test.ts` files.
  ...
  When working with `.unit.test.ts` files and Wallaby MCP tools return `<No data available>` or empty responses:
  ...
  2. **User Notification (One-Time Only)**:
     ⚠️ Wallaby server appears to be inactive or not configured for this project.

     Please ensure:
  1. Wallaby server is running (check VS Code status bar)
  2. The project is configured with wallaby.js config file
  3. Tests have been run at least once to populate Wallaby's cache
...
```
  ...
  START
    ↓
  CHECK_WALLABY_ACTIVE
    ↓
  [Inactive] → NOTIFY_USER → HALT
    ↓
  [Active] → DISCOVER_FAILING_TESTS
    ↓
  [No Failures] → REPORT_SUCCESS → END
    ↓
  [Has Failures] → ANALYZE_FIRST_FAILURE
    ↓
  GET_RUNTIME_VALUES
    ↓
  IMPLEMENT_FIX
    ↓
  VERIFY_FIX_WITH_WALLABY
    ↓
  [Still Failing] → ANALYZE_DEEPER
    ↓
  [Fixed] → NEXT_FAILURE or END

  Response Templates

  When Wallaby is working:
  ✅ Found 3 failing tests via Wallaby:
  ...
  When debugging with runtime values:
  🔍 Wallaby runtime analysis at line 42:
  ...
  When test is fixed:
  ✅ Test "should_handle_empty_input" now passing in Wallaby
